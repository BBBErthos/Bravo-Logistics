...
const EJS_CONFIRM_TPL = BRAVO_CONFIG.EJS_CONFIRM_TPL;
  ...
  try {
    ...
    const updateCol = async (colId, val) => {
      try {
        ...
        console.error(`Column ${colId} failed:`, e.message, "value:", JSON.stringify(val));
        throw e;
      }
    };
    await updateCol(DEL_COL.date,     { date: date });
    await updateCol(DEL_COL.status,   { label: "Scheduled" });
    await updateCol(DEL_COL.yard,     { label: yard });
    await updateCol(DEL_COL.timeSlot, selectedSlot.label.replace('–', '-'));

    // Send confirmation email to dispatcher
    const slotLabel = selectedSlot.label.replace("–", "-");
